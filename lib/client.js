  ...
  factory: (require) => {
    ...
    function PluginManagerSection(props) {
      ...
      return ui.h(React.Fragment, null,
        ...
        // Preset controls（0.5.3：A 折叠卡默认收起 + B 切换器仅预设数>1 时显示；间距由 kit 节奏规则统一）
        ui.h(ui.Card, null,
          ui.h('div', {
            className: 'sui-card-title',
            style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' },
            onClick: () => toggleCollapse('preset'),
          },
            ui.h('span', {}, `预设管理（当前：${activePreset}）`),
            ui.h('span', {}, collapsed.preset ? '▸' : '▾'),
          ),
          !collapsed.preset
            ? ui.h('div', { className: 'sui-card-main', style: { display: 'flex', flexDirection: 'column', gap: '12px' } },
              presets.length > 1
        ...
        // 0.5.3：B=整卡按状态显隐（无静态层包且无托管包 → 完全不占位）；A=有内容时默认折叠。
        (staticLayerPkgs.length > 0 || managedPkgs.length > 0)
          ? ui.h(ui.Card, null,
            ui.h('div', {
              className: 'sui-card-title',
              style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' },
              onClick: () => toggleCollapse('coexist'),
            },
              ui.h('span', {}, `官方共存（双轨管理）（${staticLayerPkgs.length} 静态 / ${managedPkgs.length} 托管）`),
              ui.h('span', {}, collapsed.coexist ? '▸' : '▾'),
            ),
            !collapsed.coexist
              ? ui.h('div', { className: 'sui-card-main', style: { display: 'flex', flexDirection: 'column', gap: '12px' } },
          ...
          // 静态层包 → 导入到 bm（0.5.1：可多选批量）
          staticLayerPkgs.length > 0
            ? ui.h('div', { className: 'sui-card-main', style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
              ui.h('div', { className: 'sui-card-meta' },
                ui.h('label', { style: { display: 'inline-flex', alignItems: 'center', gap: '6px', cursor: 'pointer' } },
                  ui.h('input', {
                    type: 'checkbox',
                    checked: selStatic.length === staticLayerPkgs.length && selStatic.length > 0,
                    onChange: toggleAllStatic,
                  }),
                  `官方静态层（${staticLayerPkgs.length} 个，非托管）：`,
                ),
              ),
              staticLayerPkgs.map((pkg) => ui.h('div', {
                key: pkg,
                className: 'sui-row',
              ...
              selStatic.length > 0
                ? ui.h('div', { className: 'sui-actions' },
                  ui.h('button', {
                    ...
                    className: 'sui-btn',
                    disabled: busy || saving || coexistBusy,
                    onClick: () => setCoexistAction({ kind: 'import', pkgs: selStatic }),
                  }, `批量导入到 bm（${selStatic.length}）`),
                )
                : null,
            )
            : null,
          // 托管包 → 导出 / 卸载（0.5.1：可多选批量）
            ...
            ? ui.h('div', { className: 'sui-card-main', style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
              ui.h('div', { className: 'sui-card-meta' },
                ui.h('label', { style: { display: 'inline-flex', alignItems: 'center', gap: '6px', cursor: 'pointer' } },
                  ...
                  `bm 托管（${managedPkgs.length} 个）：`,
                ),
              ),
              managedPkgs.map((row) => ui.h('div', {
                key: row.pkg,
                className: 'sui-row',
                style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' },
              },
              ...
              selManaged.length > 0
                ? ui.h('div', { className: 'sui-actions' },
                  ui.h('button', {
                    ...
                    className: 'sui-btn',
                    disabled: busy || saving || coexistBusy,
                    onClick: () => setCoexistAction({ kind: 'export', pkgs: selManaged }),
                  }, `批量导出到官方（${selManaged.length}）`),
                  ui.h('button', {
                    type: 'button',
                    className: 'sui-btn sui-btn-danger',
                    disabled: busy || saving || coexistBusy,
                    onClick: () => setCoexistAction({ kind: 'uninstall', pkgs: selManaged }),
                  }, `批量卸载（${selManaged.length}）`),
                )
                : null,
            )
            : null,
          // 全局动作：导出全部（安全网）+ 回滚导入（间距由外层卡体局部 stack 统一）
          ui.h('div', { className: 'sui-actions' },
            ui.h('button', {
              type: 'button',
              className: 'sui-btn',
