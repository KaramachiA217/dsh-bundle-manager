...
function discoverCandidates(dependencies) {
  ...
  // are ignored. 0.5.2：**整个 `@deepseek-ai` 官方 scope 跳过**——它是核心包
  // 家族（framework 白名单只是其中 4 个），其中 runner 型 bundle（如
  // dsh-headless，patch 含 code-runtime 行）被误当候选会污染列表，且导出
  // 会与基础层重复行 ID 直接 brick boot（2026-08-19 实测）。
  ...
  for (const entry of top) {
    if (entry.isDirectory() && entry.name === '@deepseek-ai') continue
...
export async function apply(ctx) {
  ...
  const exportToBundles = (pkgs) => {
    const registry = loadRegistry()
    const { bundles, candidates } = snapshot()
    const active = registry.presets[registry.activePreset] ?? {}
    const nextBundles = [...bundles]
    const exported = []
    const rejected = []
    ...
    for (const pkg of pkgs) {
      ...
      // 0.5.1：已装可选 bundle（候选）即可固化，不要求 bm 托管行——未挂载的
      // 插件「导出到官方」= 加入静态层随 dsh 启动（重启后由官方层挂载）。
      if (!(pkg in active) && !candidates.includes(pkg)) {
        rejected.push({ pkg, code: 'not-managed', message: '该包未安装且不是可选 bundle（无法固化，请先安装）' })
        continue
      }
    ...
    if (manifestChanged) writeManifestBundles(nextBundles)
    for (const pkg of exported) {
      if (pkg in active) active[pkg].state = 'superseded-by-static'
    }
    enforceCaps(registry)
    assertStorageWriteOk(saveRegistry(registry))
